        ...
        >
          ...
          <div className="p-5 flex-1 flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm text-[#5B6B7A] leading-relaxed mt-1 line-clamp-2">
...
const PaginationControls = ({
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-12">
      <Link
        href={`/blog?category=${category}&page=${Math.max(1, currentPage - 1)}`}
      ...
      <Link
        href={`/blog?category=${category}&page=${Math.min(totalPages, currentPage + 1)}`}
        className={`p-2 rounded-full border transition-all ${
...
// DATA FETCHING
// ============================================================
async function fetchPosts(category?: string, page: number = 1): Promise<FetchPostsResult> {
  const offset = (page - 1) * POSTS_PER_PAGE;
  const categoryFilter = category && category !== "all" ? `&filter[category][_eq]=${category}` : "";
  const url = `${API_URL}/items/posts?filter[status][_eq]=published&sort[]=-published_at&limit=${POSTS_PER_PAGE}&offset=${offset}${categoryFilter}`;
  const countUrl = `${API_URL}/items/posts?filter[status][_eq]=published${category ? `&filter[category][_eq]=${category}` : ""}&aggregate[count]=*`;
  ...
  try {
    ...
    if (!postsRes.ok || !countRes.ok) {
      throw new Error(`Failed to fetch posts: ${postsRes.status} ${countRes.status}`);
    }
...
export default function BlogPage() {
  const [searchParams, setSearchParams] = useState<{ category: string; page: number }>({
    category: "all",
    page: 1,
  });
  ...
  const updateParams = useCallback((newCategory?: string, newPage?: number) => {
    const cat = newCategory ?? searchParams.category;
    const page = Math.max(1, newPage ?? searchParams.page);
  ...
  return (
    ...
    >
          ...
          <ScrollReveal>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                ...
                <p className={`${mono.className} text-sm mt-2`} style={{ color: MUTE_ON_LIGHT }}>
      ...
      <CategoryFilter
        ...
        onCategoryChange={(category) => updateParams(category, 1)}
      />
