...
async function getCategoryArticles(category: string) {
  try {
    const res = await fetch(
      `${API_URL}/items/support_articles?filter[category][_eq]=${category}&filter[status][_eq]=published&sort[]=-featured&sort[]=-created_at`,
      { cache: "no-store" }
    );
    if (!res.ok) return [];
...
export default function CategoryPage() {
  ...
  const label = config?.label || categorySlug;
